  ...
  Form.prototype.val = function(filter, object){
    ...
    formElem.each(function(index, item){
      ...
      layui.each(object, function(key, value){
        ...
        //如果为复选框
        if(type === 'checkbox'){
			for(var j=0; j<itemElem.length;j++){
			  if(Array.isArray(value)){ 
				for(var i=0; i<value.length;i++){
					if(itemElem[j].value == value[i]){
						itemElem[j].checked = "true";
						console.log(itemElem[j])
					}
				}
			  }
		  }
    ...
    layui.each(fieldElem, function(_, item){ 
     ...
     //console.log(init_name)
	  ...
	  if("checkbox" == item.type){
		  if(field[init_name || item.name] == undefined){
			  field[init_name || item.name] = [];
		  }
		   field[init_name || item.name].push(item.value);
	  }else{

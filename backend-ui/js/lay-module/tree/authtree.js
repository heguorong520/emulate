    ...
    var obj = {
        ...
        render: function (dst, trees, opt) {
            ...
            if (typeof opt.theme === 'string' && opt.theme !== '') {
                ...
                layui.link(layui.cache.base +"/tree/themes/" +opt.theme + '.css')
            }
            if (opt.hidechoose) {
            ...
            $(dst).html(obj.renderAuth(trees, 0, {
				...
				checkedValueList:opt.checkedValueList,
        ...
        renderAuth: function (tree, dept, opt) {
			...
			var checkedValueList =  opt.checkedValueList||[];
            // 参数配置
            ...
            layui.each(tree, function (index, item) {
				...
				for(var i=0;i<checkedValueList.length;i++){
					console.log(checkedValueList[i]+"==="+item[opt.valueKey])
					console.log(checkedValueList[i] ===item[opt.valueKey])
					if(checkedValueList[i] == item[opt.valueKey]){
						isChecked = true;
						break;
					}
				}

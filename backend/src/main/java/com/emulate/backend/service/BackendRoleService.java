...
public class BackendRoleService extends ServiceImpl<BackendRoleDao, BackendRoleEntity> {
    ...
    public PageData findPage(QueryRoleDTO queryRoleDTO) {
        IPage<BackendRoleEntity> page =
            this.page(new Page<>(queryRoleDTO.getPage(), queryRoleDTO.getLimit()), new QueryWrapper<BackendRoleEntity>()
                .like(ObjectUtil.isNotEmpty(queryRoleDTO.getRoleName()), "role_name", queryRoleDTO.getRoleName()));
        // 查询角色对应的菜单
        if (page.getRecords().size() > 0) {
            List<Long> roleIds =
                page.getRecords().stream().map(BackendRoleEntity::getRoleId).collect(Collectors.toList());
            // 获取角色对应菜单ID
            Map<Long, List<Long>> userMenuIdMaps =
                backendRoleMenuService.findMenuIdList(roleIds.toArray(new Long[roleIds.size() - 1]));
    ...
    public Map<Long, String> findRoleNameMap(List<Long> ids) {
        ...
        result =
            roleList.stream().collect(Collectors.toMap(BackendRoleEntity::getRoleId, BackendRoleEntity::getRoleName));
        return result;
    }
